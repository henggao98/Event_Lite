...
public class EventsControllerApiTest {
	...
	@Test
	public void getEventsByNameApiTest() throws Exception {
		Venue v = new Venue();
		v.setName("Initial Venue");
		v.setCapacity(10);
		Event e = new Event();
		...
		e.setTime(LocalTime.now());
		e.setVenue(v);
		when(eventService.findByName("Event")).thenReturn(Collections.<Event>singletonList(e));

		mvc.perform(get("/api/events/search?sname=Event").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
